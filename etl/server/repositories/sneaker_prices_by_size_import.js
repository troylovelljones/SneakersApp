...
const COMMA = `,`;
const LOCK_TABLE = `LOCK TABLE sneaker_prices_by_size_import IN EXCLUSIVE MODE; `;
const BATCH_SIZE = 200;
const showLogStatements = true;
const options = {timestamps: {includeDefaultTimestampFields: false,
    fields: {created_at: 'created_at'}, dbTimeFunction: 'LOCALTIMESTAMP'}}
const sneakerPriceBySizeImport = {
    bulkCreate: async (prices) => {
        ...
        try {
                ...
                for (let batch = 0; batch < Math.ceil(prices.length / BATCH_SIZE); batch++) {
                    ...
                    sqlInsertStatement =
                        createSqlInsertStatement('sneaker_prices_by_size_import', pricesBatch, options);
...
module.exports = sneakerPriceBySizeImport;