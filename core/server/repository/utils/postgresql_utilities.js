...
const stringTool = require('string');
...
const translateValue = (value, alwaysTreatPropertyValueAsString) => {
    ...
    debug('Translated value:'.underline + ' ' + (value?.blue || `NULL`.grey.bold.underline.italic));
    return (value || `NULL`)
...
const includeTimestampsInUpdate = (updateStatement, options) => {
    ...
    debug('Update statement from includeTimeSTamps...= ' + updateStatement.magenta.bold);  
...
const updateFromProperties = (record, updateStatement, options) => {
    ...
    !updateStatement && new Error('SQL statement is blank!'.red);
    debug('Update statement from updateFromProps... = '.cyan.underline.italic + updateStatement.magenta.bold);
...
function whereClause (sqlStatement) { 
    ...
    const checkForUndefinedValues = () => {
        ...
        sqlStatement.includes('undefined') && 
            !sqlStatement.includes(`'undefined'`) && throwError('Invalid SQL Statement!'.red)
    }
    ...
    const conditionals = { 
        ...
        groupBy: (fields) => {
            ...
            debug('Got an array of fields.') : debug('Got a single field.');
            checkForDuplicateGroupBy(fields);
            Array.isArray(fields) ? debug('still dealing with an array') : debug('not an array');
        ...
        orderBy: (fields) => {
            ...
            fields = Array.isArray(fields) ? fields: [fields];
            Array.isArray(fields) ? debug('Got an array of fields.') : debug('Got a single field.');
            checkForDuplicateGroupBy(fields);
        ...
        limit: (value) => {
            ...
            debug('postion of the word limit: ' + sqlStatement.
                    indexOf(LIMIT) - 1);
            debug(sqlStatement.substr(0, sqlStatement.
                indexOf(LIMIT)));
...
const replaceTablenames = (tablenames, selectStatement) => {
    ...
    tablenames.forEach((tablename, index) => {
        ...
        selectStatement = !isLastTable ? 
            ...
            !debug(`Added last table to select`) && selectStatement.replace(TABLE, tablename);
                    
        debug(selectStatement.green);
    });
    debug(`Select statement after adding tables `.blue + `${selectStatement}`.magenta);
    return selectStatement;
} 

const replaceFields = (fieldnames, sqlSelectStatement) => {
...
const createSqlInsertWhereStatement = (tablename, records, select, where, options) => {
    ...
    debug('constructInsertWhereStatement before...'.yellow, insertStatement.green);
...
const insert_db_time_function = (sqlInsertStatement, fields, dbTimeFunction) => {
    ...
    debug(('Insert Substring up to last character: ' 
...
const record_fieldsOrValues_to_insertStatement = (record, sqlInsertStatement, options) => {
    ...
    debug('insertFromProperties after insertDBTimeFunction... '.magenta + sqlInsertStatement);

    sqlInsertStatement = options?.timestamps && insert_db_time_function(sqlInsertStatement, options.timestamps.fields, options.timestamps.dbTimeFunction) ||
            sqlInsertStatement;
...
function records_to_insertStatement(records, sqlInsertStatement, options) {
    ...
    records.forEach((record, index) => {
        ...
        debug(`insertFieldsAndValuesFromObjects before insertFromProperties... `.yellow, sqlInsertStatement);
        ...
        debug('insertFieldsAndValuesFromObjects after insertFromProperties... '.yellow, sqlInsertStatement);
...
const createSqlInsertStatement = (tablename, records, options) => {
    ...
    debug(`SQL insert statement from inserTimestampFields...${sqlInsertStatement}`);
    debug('constructInsertStatement before insertFromObjects'.yellow, sqlInsertStatement.green);
